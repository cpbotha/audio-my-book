...
async def process_chunk(chunk, speech_filename):
...
async def process_chapter(chapter_text, chapter_title, output_dir):
    ...
    async with asyncio.TaskGroup() as tg:
        for i, chunk in enumerate(text_chunker.chunk(chapter_text)):
            ...
            if speech_filename.exists():
                ...
                tasks.append(tg.create_task(process_chunk(chunk, speech_filename)))

    click.echo(f"Completed tasks for chapter {chapter_title}: {tasks}")


async def process_book(input_txt):
    ...
    with input_path.open() as f:
        ...
        for idx, chapter in enumerate(chapters):
            ...
            if fnmatch.fnmatch(chapter[1], "Chapter 1"):
                # start, end, title
        ...
        async with asyncio.TaskGroup() as tg:
            for chap_idx, chapter in enumerate(selected_chapters):
                ...
                chap_results[chap_idx] = tg.create_task(process_chapter(chapter_text, chapter[2], input_path.parent))
...
def cli(input_txt):
    ...
    loop.run_until_complete(process_book(input_txt))
